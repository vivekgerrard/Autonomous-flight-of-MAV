 ...
 */
//VERBOSE_PRINT("x: %d y: %d  \n", gmax_area_xcentre, gmax_area_ycentre);
...
//VERBOSE_PRINT("x: %d y: %d area: %f \n", gmax_area_xcentre, gmax_area_ycentre, gmax_area);
//VERBOSE_PRINT("a: %d\n", expansion);
	  float moveDistance = fmin(maxDistance, 0.05 * trajectoryConfidence);

	// If centre of the largest color blob gets updated
	  if(gmax_area_xcentre != 0)
		{
			//Thresholded area, if it exceeds the threshold, avoid obstacle
			if(gmax_area > 30000)
				{
					...
					//Splitting the image into three segments
					//Left segment
					if(0< gmax_area_xcentre < 173)
						{
	  						      waypoint_set_here_2d(WP_GOAL);
	      						      waypoint_set_here_2d(WP_TRAJECTORY);
	      						      increase_nav_heading(&nav_heading, 10.0); //5.0 initially
	      						      if(trajectoryConfidence > 7)
								{
					...
					else if(173<= gmax_area_xcentre < 350) //Middle segment with increased heading
						{
	  						      waypoint_set_here_2d(WP_GOAL);
	      						      waypoint_set_here_2d(WP_TRAJECTORY);
			...
			else
			{
		     	 ...
		     	 chooseRandomIncrementAvoidance();
		     	 trajectoryConfidence += 1;
		     	 if(trajectoryConfidence>20)
		     	 {
		     		 trajectoryConfidence = 20;
		     	 }
			}
	  return;
	}
...
uint8_t chooseRandomIncrementAvoidance()
{
  ...
  int r = rand() % 2;
  if (r == 0) {
    incrementForAvoidance = 70.0;  //60.0 initially
    //VERBOSE_PRINT("Set avoidance increment to: %f\n", incrementForAvoidance);
  } else {
    incrementForAvoidance = -70.0;
    //VERBOSE_PRINT("Set avoidance increment to: %f\n", incrementForAvoidance);
  }
  return false;
}
